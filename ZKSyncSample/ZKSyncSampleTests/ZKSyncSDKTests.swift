...
class ZKSyncSDKTests: XCTestCase {
    ...
    func testGetPublicKey() {
        ...
        guard case let .success(publicKey) = ZKSyncSDK.getPublicKey(privateKey: privateKey) else {
    ...
    func testGetPublicKeyHash() {
        ...
        guard case let .success(publicKeyHash) = ZKSyncSDK.getPublicKeyHash(publicKey: publicKey) else {
    ...
    func testSignMessage() {
        ...
        guard case let .success(signature) = ZKSyncSDK.signMessage(privateKey: privateKey, message: message) else {
    ...
    func testErronOnSeedTooShort() {
        ...
        guard case let .failure(error) = ZKSyncSDK.generatePrivateKey(seed: seed) else {
            XCTFail("Incorrect private key generated")
            return
        }
        
        XCTAssertEqual(error as! ZKSyncSDKError, ZKSyncSDKError.seedTooShortError)
    }
    ...
    func testErrorOnMusigMessageTooLong() {
        ...
        guard case let .failure(error) = ZKSyncSDK.signMessage(privateKey: privateKey, message: message) else {
            XCTFail("Incorrect signature generated")
            return
        }

        XCTAssertEqual(error as! ZKSyncSDKError, ZKSyncSDKError.musigTooLongError)
    }
}
